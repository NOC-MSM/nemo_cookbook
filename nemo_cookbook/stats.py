...
def compute_binned_statistic(
    ...
    """
    # -- Validate input -- #
    if not isinstance(vars, list) or not all(isinstance(var, xr.DataArray) for var in vars):
    ...
    if keep_dims is not None:
        if not isinstance(keep_dims, list) or not all(isinstance(dim, str) for dim in keep_dims):
            raise ValueError("keep_dims must be a list of strings or None.")
    if not isinstance(bins, list) or not all(isinstance(bin, (list, np.ndarray)) for bin in bins):
    ...
    coord_dict = {f'{var.name}_bins': np.array([interval.mid for interval in da[f'{var.name}_bins'].values]) for var in vars}
